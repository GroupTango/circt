//===- HWModelExplorerInterfaces.cpp - Implement HWModelExplorerInterfaces
//------*- C++ -*-===//
//
...
std::string hw::MlirToInstanceGraphJson(mlir::Operation *baseModule,
                                        raw_ostream *os) {
        ...
        llvm::TypeSwitch<mlir::Operation *>(&op)
            ...
            .Default([&](auto) {
              ...
              modulesToProcess.push(
                  {nullptr, op.getName().getStringRef().str()});
            });
      }
    }
  }
  ...
  while (modulesToProcess.size() > 0) {
        ...
        for (mlir::Operation &op : block.getOperations()) {
          if (auto instance = dyn_cast<InstanceOp>(&op)) {
            std::string newNamespace =
                nextPair.second + "/" +
                instance.getReferencedModuleName().str() + " (I" +
                std::to_string(instanceID) + ")";

            auto it = moduleMap.find(instance.getReferencedModuleName());
            if (it != moduleMap.end())
              modulesToProcess.push(
                  {moduleMap[instance.getReferencedModuleName()],
                   newNamespace});
            else
            ...
            for (auto attr : moduleNames) {
              mlir::StringRef instanceName =
                  llvm::cast<FlatSymbolRefAttr>(attr).getValue();
              std::string newNamespace =
                  nextPair.second + "/INSTANCE CHOICE (I" +
                  std::to_string(instanceID) + ")/" + instanceName.str();
    ...
    if (!hasInstances) {
      ...
      llvm::TypeSwitch<mlir::Operation *>(module)
          .Case<hw::HWModuleOp>(
              [&](auto module) { moduleJson["label"] = "Self-Contained"; })
          .Case<hw::HWModuleExternOp>(
              [&](auto module) { moduleJson["label"] = "External"; })
          .Case<hw::HWModuleGeneratedOp>([&](auto module) {
            moduleJson["label"] = "External (Generated)";
          })
          .Default([&](auto) { moduleJson["label"] = "UNKNOWN ERROR"; });
...
std::string getUniqueId(mlir::Operation *node, uint64_t &nextNodeId,
                        std::string namesp) {
  if (node == nullptr)
...
llvm::json::Array
getIncomingEdges(NodeRef node, hw::HWModuleOp module,
                 llvm::DenseMap<NodeRef, std::vector<NodeRef>> &incomingEdges,
                 uint64_t &nextNodeId, std::string namesp) {
  llvm::json::Array edges;
  for (NodeRef parent : incomingEdges[node]) {
    edges.push_back(llvm::json::Object{
        {"sourceNodeId", getUniqueId(parent, nextNodeId, namesp)},
...
std::string hw::MlirToOperationGraphJson(mlir::Operation *baseModule,
                                         raw_ostream *os) {
  llvm::StringMap<mlir::Operation *> moduleMap;
  std::stack<std::pair<mlir::Operation *, std::string>> modulesToProcess;
  ...
  while (modulesToProcess.size() > 0) {
        ...
        for (mlir::Operation &op : block.getOperations()) {
          ...
          if (auto instanceOp = dyn_cast<InstanceOp>(op)) {
            ...
            jsonObj["incomingEdges"] = llvm::json::Array{llvm::json::Object{
                {"sourceNodeId",
                 getUniqueId(moduleMap[refModuleName], nextNodeId,
                             nextPair.second + "/" + refModuleName.str())},
                {"sourceNodeOutputId", "0"},
                {"targetNodeInputId", "0"}}};
          } else {
            // intra-module dependency, get from module graph
            jsonObj["incomingEdges"] = getIncomingEdges(
                &op, moduleOp, incomingEdges, nextNodeId, nextPair.second);
          }
    ...
    if (!hasInstances) {
      ...
      llvm::json::Object moduleJson{
          ...
          {"incomingEdges", getIncomingEdges(module, moduleOp, incomingEdges,
                                             nextNodeId, nextPair.second)}};
